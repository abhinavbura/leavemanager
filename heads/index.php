				...
				<div class="row align-items-center">
					...
					<div class="col-md-8">
						...
						<?php $query= mysqli_query($conn,"select * from tblemployees where emp_id = '$session_id'")or die(mysqli_error());
								$row = mysqli_fetch_array($query);
								$dep=$row['Department'];
								$lol= mysqli_query($conn,"select DepartmentName from tbldepartments where DepartmentShortName = '$dep'")or die(mysqli_error());
								$low=mysqli_fetch_array($lol);
						?>

						<h4 class="font-20 weight-500 mb-10 text-capitalize">
							Welcome back <div class="weight-600 font-30 text-blue"><?php echo $row['FirstName']. " " .$row['LastName']; ?>,</div>
						</h4>
						<p class="font-18 max-width-600">head of the department <?php echo $low['DepartmentName']?></p>
					</div>
				</div>
			</div>
								...
								<td>
									<div class="dropdown">
										...
										<div class="dropdown-menu dropdown-menu-right dropdown-menu-icon-list">
											...
											<a class="dropdown-item" href="haccept.php?leaveid=<?php echo $result->lid;?>&tracker=2"><i class="dw dw-eye"></i>decline</a> <?php } ?>
										</div>
									</div>
								</td>
							</tr>
